...
* **更新ロジック**: 
  `新たなトークン数 = min(最大容量, 現在のトークン数 + 経過時間 × 補充レート)`
* **設定**: 0.2 token/sec（補充）、3 tokens（最大貯蔵）
* **意図**: 突発的なアクセスによるCPUリソースの枯渇を防止し、最低限の可用性を死守します。

### Middlewareによる階層防御
`TokenBucketMiddleware` により、Viewが実行される前の段階でリクエストを評価します。
